  ...
  try {
    ...
    switch (req.method) {
      case "GET":
        ...
        } else if (url.searchParams.has("getAll")) {
...
const getAllChallenges = async () => {
  const response = await supabaseFetch(
    `${supabaseUrl}/rest/v1/challenges?select=*,challenge_tags(tag_id, tags(name))`,
    { method: "GET" }
  );
...
// Fetch challenges filtered by tags (GET)
const getChallenges = async (selectedTags: string[]) => {
  ...
  if (selectedTags.length > 0) {
    // Fetch challenges that have any of the selected tags (inner join)
    ...
    // Fetch all challenges with their tags using left join

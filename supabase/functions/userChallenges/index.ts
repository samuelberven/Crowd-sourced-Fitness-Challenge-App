...
const corsHeaders = {
  ...
  "Access-Control-Allow-Origin": "*", // Replace "*" with frontend URL once deployed to ensure security
...
const createUserChallenge = async (req: Request) => {
  try {
    ...
    const user_id = userData[0].id; // Assumes userData contains the 'id' field
    ...
    const response = await supabaseFetch(
      `${supabaseUrl}/rest/v1/users_challenges`,
      {
        ...
        body: JSON.stringify({
          ...
          completed: body.completed || false, // Default to false if not provided (just like in the DB)
          favorites: body.favorites || false, // Default to false if not provided (just like in the DB)
        }),
      }
    );
...
const updateUserChallenge = async (id: string, req: Request) => {
  try {
    ...
    // Check that at least one user-challenges field (completed or favorites) is provided for updating
...
const handleRequest = async (req: Request) => {
  ...
  try {
    switch (req.method) {
      ...
      case "POST":
        return await createUserChallenge(req);

      case "PATCH":
        if (id) {
          return await updateUserChallenge(id, req);
        }
